...
export function MiniStandings({ standings }: { standings: TeamStanding[] }) {
  return (
    <div className="overflow-hidden">
      {/* Header */}
      <div className="grid grid-cols-[24px_1fr_28px_28px_28px_40px_44px] gap-2 px-4 py-2 bg-gray-800/50 text-gray-500 text-xs font-semibold uppercase tracking-wide">
        <span>#</span>
        <span>Team</span>
      ...
      {standings.map((s, i) => (
        <Link
          ...
          href={`/teams/${s.teamId}`}
          className={cn(
            "grid grid-cols-[24px_1fr_28px_28px_28px_40px_44px] gap-2 px-4 py-3 items-center border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors text-sm",
            i === 0 && "bg-yellow-500/5",
          )}
        >
          ...
          <span className="text-center text-gray-400">{s.wins}</span>
          <span className="text-center text-gray-400">{s.losses}</span>
          <span
            className={cn(
